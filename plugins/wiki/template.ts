...
export async function generateWikiMD(wiki: IWiki) {
  const localesGenerate = supportLocales.map((locale) => {
    ...
    const docDir = `${projectRoot}/wiki/${locale}`;
    if (!fs.existsSync(docDir)) {
      fs.mkdirSync(docDir, { recursive: true });
    }
    const docPath = `${docDir}/${wiki.slug}.md`;
    // console.log(`-> create doc: ${docPath}`, wikiUtils.toPage());
    const docContent = Mustache.render(template, wikiUtils.toPage());
